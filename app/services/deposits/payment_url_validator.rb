...
module Deposits
  ...
  class PaymentUrlValidator < ApplicationService
    ...
    def check_currency!
      return if SafeCharge::Currency::AVAILABLE_CURRENCY_LIST
    ...
    def check_amount!
      return if query.total_amount.to_f.positive? &&
    ...
    def time_stamp_valid?
      Time
        .strptime(query.time_stamp, GetPaymentPageUrl::TIME_STAMP_FORMAT)
        .present?
    ...
    def date_of_birth_valid?
      query.dateOfBirth.nil? ||
        Date.strptime(query.dateOfBirth,
                      GetPaymentPageUrl::DATE_OF_BIRTH_FORMAT).present?
    rescue ArgumentError, TypeError
      false
    end
