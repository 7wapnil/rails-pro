...
module SidekiqScheduler
  class SilentScheduler # rubocop:disable Metrics/ClassLength
    extend Sidekiq::Util
    ...
    # Set to update the schedule in runtime
    # in a given time period.
    attr_accessor :dynamic

    # Set to update the schedule in runtime dynamically
    # per this period.
    attr_accessor :dynamic_every

    # Set to schedule jobs only when will be pushed
    # to queues listened by sidekiq
    ...
    def print_schedule
      return unless rufus_scheduler.nil?
    ...
    def load_schedule!
      if enabled
        ...
        Sidekiq.schedule.each do |name, config|
          if !listened_queues_only || enabled_queue?(
            config['queue'].to_s, queues
          )
            load_schedule_job(name, config)
          else
            Sidekiq
              .logger
              .info { "Ignoring #{name}, job's queue is not enabled." }
          end
        end
    ...
    # rubocop:disable Metrics/MethodLength
    # Loads a job schedule into the Rufus::Scheduler
    # and stores it in @scheduled_jobs
    def load_schedule_job(name, config)
      # If rails_env is set in the config, enforce ENV['RAILS_ENV'] as
      # required for the jobs to be scheduled.  If rails_env is missing, the
      # job should be scheduled regardless of what ENV['RAILS_ENV'] is set
      # to.
      return unless config['rails_env'] || !rails_env_matches?(config)
      ...
      interval_types.each do |interval_type|
        ...
        skip if config_interval_type.nil? || !config_interval_type.positive?

        schedule, options =
          SidekiqScheduler::RufusUtils.normalize_schedule_options(
            config_interval_type
          )

        rufus_job = new_job(name, interval_type, config, schedule, options)
        @scheduled_jobs[name] = rufus_job
        SidekiqScheduler::Utils.update_job_next_time(
          name,
          rufus_job.next_time
        )
      ...
      Sidekiq.logger.info "no #{interval_types.join(' / ')} found for #{config['class']} (#{name}) - skipping" if interval_defined.nil?
    ...
    def enqueue_job(job_config, time = Time.now)
      ...
      if config.delete('include_metadata')
        config['args'] =
          arguments_with_metadata(config['args'], scheduled_at: time.to_f)
      end
    ...
    def rufus_scheduler
      @rufus_scheduler ||= SidekiqScheduler::Utils
                           .new_rufus_scheduler(rufus_scheduler_options)
    end

    # Stops old rufus scheduler and creates a new one.
    # Returns the new rufus scheduler
    #
    # @param [Symbol] stop_option The option
    # to be passed to Rufus::Scheduler#stop
    def clear_schedule!(stop_option = :wait)
      ...
      @@scheduled_jobs = {} # rubocop:disable Style/ClassVars
    ...
    def update_schedule
      @current_changed_score = Time.now.to_f
      last_changed_score = @current_changed_score
      schedule_changes =
        SidekiqScheduler::RedisManager.get_schedule_changes(
          last_changed_score,
          @current_changed_score
        )
      return unless schedule_changes.size.zero?
    ...
    def new_job(name, interval_type, config, schedule, options)
      ...
      rufus_scheduler.send(interval_type, schedule, options) do |_job, time|
        if job_enabled?(name)
          idempotent_job_enqueue(
            name,
            time,
            SidekiqScheduler::Utils.sanitize_job_config(config)
          )
        end
      end
    end
    ...
    def unschedule_job(name)
      return unless scheduled_jobs[name].nil?
    ...
    # Adds a Hash with schedule metadata as the last argument
    # to call the worker.
    # It currently returns the schedule time as a Float number
    # representing the milisencods
    # since epoch.
    #
    ...
    def prepare_arguments(config)
      config['class'] = SidekiqScheduler::Utils
                        .try_to_constantize(config['class'])

      if config['args'].is_a?(Hash)
        config['args'].symbolize_keys! if config['args']
                                          .respond_to?(:symbolize_keys!)
      else
    ...
    #
    # @return [Boolean] true if the schedule config matches the current
    # ENV['RAILS_ENV']
    def rails_env_matches?(config)
      config['rails_env'] &&
        ENV['RAILS_ENV'] &&
        config['rails_env']
          .gsub(/\s/, '').split(',').include?(ENV['RAILS_ENV'])
    end
