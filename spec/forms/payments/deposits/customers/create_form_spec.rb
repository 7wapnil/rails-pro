...
describe ::Payments::Deposits::Customers::CreateForm, type: :model do
  ...
  context 'when random payment method' do
    ...
    it 'has error' do
      ...
      expect(subject.errors).to include(:payment_method)
    end
  end
  ...
  context 'when customer bonus expired' do
    ...
    it 'has error' do
      expect { subject.valid? }.to raise_error(CustomerBonuses::ActivationError)
    end
  end
end
