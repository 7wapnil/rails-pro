...
describe Customers::LimitsCollector do
  ...
  context 'when there are found and built limits' do
    ...
    context 'entries are additionally ordered' do
      ...
      let(:ordered_extracted_titles_for_built_limits) do
        extracted_titles_for_built_limits.sort_by(&:name)
      end

      it 'by title name' do
        expect(ordered_extracted_titles_for_built_limits).to eq(ordered_titles)
      end
    end
  end
end
