describe 'Customers#betting_limits' do
  ...
  context 'global betting limit form' do
    it 'shows global betting limit form' do
      expect(page).to have_selector '.global-betting-limit-form'
    end

    it 'creates customer global betting limit' do
      visit page_path

      within 'form.global-betting-limit-form' do
      ...
      within '.container' do
        expect_to_have_notification I18n.t(
          'entities.betting_limit.flash.created'
        )
      end
    end
    ...
    it 'updates customer global betting limit' do
      ...
      within '.container' do
        expect_to_have_notification I18n.t(
          'entities.betting_limit.flash.updated'
        )
      end
    end
  end
  ...
  context 'betting limit by sport form' do
    it 'shows betting limit by sport form' do
      expect(page).to have_selector(
        ".betting-limit-by-title-#{title.id}-form"
      )
    end
    ...
    it 'creates customer betting limit by sport' do
      ...
      within "form.betting-limit-by-title-#{title.id}-form" do
